...
namespace Platinum.Weapon
{
    public class OverheatBehavior : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            DebugUtility.HandleErrorIfNullGetComponent<SkillController, OverheatBehavior>(weapon, this, gameObject);
        ...
        void Update()
        {
            ...
            float currentAmmoRatio = weapon.activeBullet.currentBulletsRatio;
            if (weapon.activeBullet.currentBulletsRatio != weapon.activeBullet.currentBulletsRatio)
            {
            ...
            if (CoolingCellsSound)
            {
                if (!m_AudioSource.isPlaying
                    ...
                    && weapon.activeBullet.IsCooling)
                {
                    m_AudioSource.Play();
                }
                else if (m_AudioSource.isPlaying
                         && (currentAmmoRatio == 1 || !weapon.IsWeaponActive || !weapon.activeBullet.IsCooling))
                {
                    m_AudioSource.Stop();
                    return;
                }
